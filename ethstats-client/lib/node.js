...
{
	console.error("No instance name specified! XXX");
	process.exit(1);
}
...
Node.prototype.startWeb3Connection = function() {
    ...
    var GETH_URL = 'ws://' + (process.env.RPC_URL || 'localhost') + ':8546'; // WebSocket URL
    console.info("GETH_URL: " + GETH_URL);
    
	const wsProvider = new WebsocketProvider('ws://geth:8546');
    ...
    if (!this._web3) {
        ...
        console.info("Web3 initialized successfully.");
    }
...
Node.prototype.setFilters = function() {
    ...
    if (!this._web3) {
        console.error("Web3 instance is not initialized. Cannot set filters.");
        return; // Exit if Web3 is not available
    }
    ...
    try {
        this.chainFilter = this._web3.eth.subscribe('newBlockHeaders', function(error, blockHeader) {
            if (error) {
                ...
                self._latestQueue.push(blockHeader.number);
...
Node.prototype.checkWeb3Connection = function() {
    ...
    this._web3.eth.net.isListening()
        .then(() => {
            console.info('Web3 connection established');
        	self.init(); // Call init after successful connection
        })
        .catch((err) => {
            console.error('Web3 connection failed:', err);
            // Implement your retry logic if needed
            this.reconnectWeb3(); // Add retry mechanism here if desired
        });
};
...
Node.prototype.getLatestBlock = function () {
    ...
    if (this._web3) {
        ...
        this._web3.eth.getBlock('latest', false, function(error, result) {
            self.validateLatestBlock(error, result, timeString);
        });
    } else {
        console.error('Web3 instance is not initialized. Cannot fetch latest block.');
    }
}
